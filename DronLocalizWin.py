...
def main(r, h):
    ...
    if r:   
        ...
        class FontSizeParamType(click.ParamType):  # to select font size 
            ...
            def convert(self, user_input, param, ctx):
                try:
                    ...
                    self.fail(f"{user_input} rozmiar czcionki musi być liczbą całkowitą!", param, ctx)
                if font_size < min_font_size or font_size > max_font_size:
                    self.fail(f" rozmiar czcionki musi być liczbą całkowitą z zakresu {min_font_size}-{max_font_size}", param, ctx)
                else:
                    print(f' wybrano czcionkę o rozmiarze {font_size}\n')    
                return font_size
        ...
        deployment_text = " rozmieszczenie może być jednym z nastepujących: gora-lewa, gora-srodek, gora-prawa, srodek-lewa, srodek-srodek, srodek-prawa, dol-lewa, dol-srodek, dol-prawa"   
        ...
        font_size = click.prompt("Wpisz rozmiar czcionki", default='50', type=FontSizeParamType())
        print(color_text)
        font_color = click.prompt("Wpisz kolor czcionki", default='white', type=FontColorParamType())
        ...
        if len(note) > 0:
            note_font_size = click.prompt("Wpisz rozmiar czcionki dla informacji dodatkowej", default='50', type=FontSizeParamType())
            note_font_color = click.prompt("Wpisz kolor czcionki dla informacji dodatkowej", default='white', type=FontColorParamType())
